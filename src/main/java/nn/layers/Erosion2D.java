...
public class Erosion2D extends Layer {
    ...
    public Erosion2D(float[][][][] w, Padding padding) {
        this.w = w;
        this.padding = padding;
    }
    ...
    private synchronized void erosion(float[][][][] y, float[][][] x, int yId, int yId1, int yId2) {
        for (int i = 0; i < w[0][0][0].length; i++) {
            y[yId][yId1][yId2][i] = Erosion.operation(x, w, new int[]{yId1 * strides[0], yId2 * strides[1]}, i);
        }
    }

    private synchronized void iterateColumns(float[][][][] y, float [][][] x, int yId, int yId1) {
        for (int i = 0; i < y[0][0].length; i++) {
            erosion(y, x, yId, yId1, i);
        }
    }
    ...
    public float[][][][] predict(float[][][][] x) {
        if (padding.equals(Padding.SAME)) { x = nn.utils.Padding.create(x, w, Float.POSITIVE_INFINITY); }
