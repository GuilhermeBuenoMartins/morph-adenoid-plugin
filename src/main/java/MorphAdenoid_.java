...
public class MorphAdenoid_ implements PlugIn {
         ...
         * int[] inputShape =
         * Arrays.stream(Config.getProperties().getProperty("model.input_shape").split(
         * ","))
         * .mapToInt(s -> Integer.parseInt(s)).toArray();
         * ImageProcessor xIp = ip.resize(inputShape[0], inputShape[1]);
         * float[][][][] x = new float[1][inputShape[0]][inputShape[1]][3];
         * return Basics.create(x, convertPixels(new float[inputShape[0] * inputShape[1]
         * * 3], xIp));
         * }
         * 
         ...
         * boolean cls = score >=
         * Float.parseFloat(Config.getProperties().getProperty("model.decision_boundary"
         * ));
         * String title =
         * String.format("Image classified in %d ms as \"%s\" with score = %f.",
         * endTime - startTime,
         ...
         * {
         * IntStream.range(0, imp.getStackSize()).parallel().filter(i -> i % step ==
         * 0).forEach(i -> {
         ...
         * "Image classified in %d ms as \"Positive\" with score = %f.",
         * endTime - startTime,
         * score);
        ...
        public void run(String arg) {
                ...
                boolean ok = IJ.showMessageWithCancel("Morph Adenoid",
                                "Initializing plugin. Are you sure to continue?");
                if (ok) {
                        classify(imp);
                }
                // if (model == null) {
                // model = Config.loadModel();
                // }

                // ImagePlus imp = IJ.getImage();
                // boolean ok = IJ.showMessageWithCancel("Morph Adenoid", "Initializing
                // plugin.");
                // if (ok) {
        ...
        private void classify(ImagePlus imp) {
                if (imp.hasImageStack() && imp.isRGB()) {
                        classifyStack(imp);
                        IJ.showMessage("The video was classified successful.");
                } else if (imp.isRGB()) {
        ...
        private void classifyStack(ImagePlus imp) {
                float bound = Float.parseFloat(Config.getProperties().getProperty("model.decision_boundary"));
                int step = getStep(imp);
                IntStream.range(0, imp.getStackSize()).parallel().filter(i -> i % step == 0).forEach(i -> {
                        ImageProcessor ip = imp.getImageStack().getProcessor(i + 1);
                        long startTime = System.currentTimeMillis();
                        float score = ((float[][]) Config.getModel().predict(getX(ip)).getObject())[0][0];
                        long endTime = System.currentTimeMillis();
                        if (score >= bound) {
                                String title = String.format(
                                                "Image classified in %d ms as \"Positive\" with score = %f.",
        ...
        public static int getStep(ImagePlus imp) {
                ...
                gd.addMessage("Please, set the step value for frame analyzing. In case of error, "
                                + "it will be consider the value " +
                                Config.getProperties().getProperty("step.default")
                                + ".");
        ...
        private void classifyImage(ImagePlus imp) {
                float bound = Float.parseFloat(Config.getProperties().getProperty("model.decision_boundary"));
                long startTime = System.currentTimeMillis();
                float score = ((float[][]) Config.getModel().predict(getX(imp.getProcessor())).getObject())[0][0];
                long endTime = System.currentTimeMillis();
                String title = String.format("Image classified in %d ms as \"%s\" with score = %f.",
                                endTime - startTime,
                                score >= bound ? "Positive" : "Negative",
                                score);
                imp.setTitle(title);
        }

        private FNArray getX(ImageProcessor ip) {
                int[] inputShape = Arrays
                                .stream(Config.getProperties().getProperty("model.input_shape").split(Config.SEP))
                                .mapToInt(s -> Integer.parseInt(s)).toArray();
                ImageProcessor resized = ip.resize(inputShape[0], inputShape[1]);
                float[] values = new float[inputShape[0] * inputShape[1] * 3];
                int[] pixels = (int[]) resized.getPixels();
                int chLength = inputShape[0] * inputShape[1];
                ...
                return FNArrayFactory.create(new int[] { 1, inputShape[0], inputShape[1], 3 }, values);
        }
